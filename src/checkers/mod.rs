...
impl Checkers {
	...
	fn action_to_positions(&self, action: &usize) -> Vec<BoardPosition> {
		...
		let y0 = from % 4;
		positions.push(
			BoardPosition::new(x0, y0 * 2 + if x0 % 2 == 0 {0} else {1})
		);
        let x1 = to / 4;
		let y1 = to % 4;
		positions.push(
			BoardPosition::new(x1, y1 * 2 + if x1 % 2 == 0 {0} else {1})
		);
		positions
	}
	...
	fn simple_move_to_action(&self, simple_move: &SimpleMove) -> usize {
		let mut positions = Vec::new();
		positions.push(
			BoardPosition::new(simple_move.from_row(), simple_move.from_column())
		);
		positions.push(
			BoardPosition::new(simple_move.to_row(), simple_move.to_column())
		);
	...
	fn jump_move_to_action(&self, jump_move: &JumpMove) -> usize {
		let mut positions = Vec::new();
		positions.push(
			BoardPosition::new(jump_move.from_row(), jump_move.from_column())
		);
		positions.push(
			BoardPosition::new(
	...
	fn legal_actions(&self) -> Vec<usize> {
		...
		if available_jump_moves.is_empty() {
			available_simple_moves.iter().map(|positions| self.simple_move_to_action(positions)).collect()
		} else {
			available_jump_moves.iter().map(|positions| self.jump_move_to_action(positions)).collect()
		}
	}
	...
	fn get_state(&self) -> (u8, Vec<Vec<u8>>, bool) {
		...
		(self.to_play(), self.get_int_board(values), self.game.is_game_over())
	}
