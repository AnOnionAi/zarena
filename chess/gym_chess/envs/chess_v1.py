...
class ChessEnvV1(gym.Env):
    def __init__(self, player_color=WHITE, log=True):
        # constants
        self.log = log
        self.initial_state = DEFAULT_BOARD
    ...
    def reset(self):
        ...
        self.white_king_on_the_board = len(
            numpy.where(self.state == KING_ID)[0]) != 0
        self.black_king_on_the_board = len(
            numpy.where(self.state == -KING_ID)[0]) != 0
    ...
    def step(self, action):
        ...
        if self.king_is_checked(state=self.state, player=opponent_player):
            self.done = True
            reward += WIN_REWARD
        if self.king_is_checked(state=self.state, player=self.current_player):
            self.done = True
            reward += LOSS_REWARD
        else:
            reward = 0
    ...
    def player_move(self, action):
        ...
        new_state, reward = self.next_state(
            self.state, self.current_player, move, commit=True
        )
        ...
        if type(move) is str and move in CASTLE_MOVES:
            ...
            if ID_TO_TYPE[piece_to_move] == PAWN:
                if (player == WHITE and _to[0] == 7) or (
                    ...
                    new_state[_to[0], _to[1]] = QUEEN_ID * \
                        self.player_to_int(player)
    ...
    def state_to_grid(self):
        grid = [[f" {ID_TO_ICON[square]} " for square in row]
    ...
    def action_to_move(self, action):
        ...
        x1, y1 = _to // 8, _to % 8
        return [
            numpy.array([x0, y0], dtype=numpy.int8),
            numpy.array([x1, y1], dtype=numpy.int8),
        ]
    ...
    def get_possible_moves(
        self, state=None, player=None, attack=False, skip_pawns=False
    ):
        if state is None:
            state = self.state
        if player is None:
        ...
        if not attack:
            ...
            squares_under_attack = self.get_squares_attacked_by_player(
        ...
        moves = []
        for coords, piece_id in numpy.ndenumerate(state):
            coords = numpy.array(coords, dtype=numpy.int8)
            if piece_id == 0:
                continue
            ...
            if piece_type == KING:
                ...
                moves += self.queen_moves(player,
                                        coords, state=state, attack=attack)
            elif piece_type == ROOK:
                moves += self.rook_moves(player, coords,
                                        state=state, attack=attack)
            elif piece_type == BISHOP:
                moves += self.bishop_moves(player,
                                        coords, state=state, attack=attack)
            elif piece_type == KNIGHT:
                moves += self.knight_moves(player,
                                        coords, state=state, attack=attack)
            elif piece_type == PAWN and not skip_pawns:
                moves += self.pawn_moves(player, coords,
                                        state=state, attack=attack)
    ...
    def king_moves(
        ...
        if attack:
            for step in steps:
                square = coords + numpy.array(step, dtype=numpy.int8)
    ...
    def rook_moves(self, player, coords, state=None, attack=False):
        ...
        for step in [[-1, 0], [+1, 0], [0, -1], [0, +1]]:
            moves += self.iterativesteps(player,
                                        state, coords, step, attack=attack)
        return moves
    ...
    def iterativesteps(self, player, state, coords, step, attack=False):
        ...
        moves = []
        k = 1
        step = numpy.array(step, dtype=numpy.int8)
        while True:
            square = coords + k * step
            if attack:
                add_bool, stop_bool = self.attacking_move(
                    player, state, square)
                if add_bool:
                    moves.append([coords, square])
                if stop_bool:
    ...
    def pawn_moves(self, player, coords, state=None, attack=False):
        ...
        attack_squares = [
            coords + numpy.array([1, -1], dtype=numpy.int8) * (-player_int),
            coords + numpy.array([1, +1], dtype=numpy.int8) * (-player_int),
        ]
        one_step_square = coords + \
            numpy.array([1, 0], dtype=numpy.int8) * (-player_int)
        two_step_square = coords + \
            numpy.array([2, 0], dtype=numpy.int8) * (-player_int)

        if attack:
            for square in attack_squares:
                if ChessEnvV1.square_is_on_board(
                    square
                ) and not self.is_king_from_player(player, state, square):
                    moves.append([coords, square])
        else:
            ...
            for square in attack_squares:
                if ChessEnvV1.square_is_on_board(
                    square
                ) and self.is_piece_from_other_player(player, state, square):
                    moves.append([coords, square])
    ...
    def get_squares_attacked_by_player(self, state, player):
        moves = self.get_possible_moves(
            state=state, player=player, attack=True)
    ...
    def king_is_checked(self, state=None, player=None):
        ...
        king_pos = numpy.where(state == king_id)
        king_square = [king_pos[0][0], king_pos[1][0]]
        other_player = self.get_other_player(player)
        attacked_squares = self.get_squares_attacked_by_player(
            state, other_player)
        if not attacked_squares:
            return False
        return any(numpy.equal(attacked_squares, king_square).all(1))
